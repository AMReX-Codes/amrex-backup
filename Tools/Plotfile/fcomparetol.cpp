...
int main_main()
{
    ...
    while (farg <= narg) {
        ...
        } else if (fname == "-t" or fname == "--tol") {
            err_tol = std::stod(amrex::get_command_argument(++farg));
    ...
    if (plotfile_a.empty() and plotfile_b.empty()) {
        amrex::Print()
            ...
            << "\n"
            << " usage:\n"
            << "    fcompare [-g|--ghost] [-n|--norm num] [-d|--diffvar var] [-z|--zone_info var] [-a|--allow_diff_grids] [-t|--tol num] file1 file2\n"
            << "\n"
            << " optional arguments:\n"
            << "    -g|--ghost            : compare the ghost cells too (if stored)\n"
            << "    -n|--norm num         : what norm to use (default is 0 for inf norm)\n"
            << "    -t|--tol num          : what tolerance to use for comparison (default is machine precision)\n"
    ...
    if (global_error <= err_tol and !any_nans) {
