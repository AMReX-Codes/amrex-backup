...
VisMF::Initialize ()
{
    ...
    async_comm = MPI_COMM_NULL;
    async_window = MPI_WIN_NULL; 
...
VisMF::SetNOutFiles (int newoutfiles, MPI_Comm comm)
{
    ...
    // amrex::AllPrint() << myproc << ": myfile = " << myfile << ", async_spot = " << ParallelDescriptor::MyProc(async_comm) << std::endl;
#endif
}
...
VisMF::WriteAsyncMPIComm (const FabArray<FArrayBox>& mf, const std::string& mf_name)
{
         ...
         -> WriteAsyncStatus
    {
        ...
        if (ispot)
        {
            ParallelDescriptor::Recv<int>(nullptr, 0, ispot-1, asyncTag, async_comm);
//            amrex::AllPrint() << myproc << " recv from " << myproc-1 << ". ifile/ispot/iamlast: " << ifile << "/" << ispot << "/" << iamlast << std::endl;
        }
        ...
        {
//            amrex::AllPrint() << myproc << " send to " << myproc+1 << ". ifile/ispot/iamlast: " << ifile << "/" << ispot << "/" << iamlast << std::endl;
            ParallelDescriptor::Send<int>(nullptr, 0, ispot+1, asyncTag, async_comm);
        }
...
VisMF::WriteAsyncMPIWait (const FabArray<FArrayBox>& mf, const std::string& mf_name)
{
         ...
         -> WriteAsyncStatus
    {
        ...
        if (ispot)
        {
            ...
            ParallelDescriptor::Arecv<int>(nullptr, 0, ispot-1, asyncTag, async_comm);
            icanwrite.wait();
        }
        ...
        if (!iamlast)
        {
            ...
            ParallelDescriptor::Asend<int>(nullptr, 0, ispot+1, asyncTag, async_comm);
            youcanwrite.wait();
        }
...
VisMF::WriteAsyncMPIBarrier (const FabArray<FArrayBox>& mf, const std::string& mf_name)
{
         ...
         -> WriteAsyncStatus
    {
        ...
        {
            ParallelDescriptor::Barrier(async_comm);
            barrier_count++;
        }
...
*/
         ...
         -> WriteAsyncStatus
    {
        ...
        while (barrier_count < ispot)
        {
            ...
            ParallelDescriptor::Abarrier(async_comm);
         ...
         -> WriteAsyncStatus
    {
        ...
        while (barrier_count < ispot)
        {
            waiting_reqs[barrier_count++] =
                ParallelDescriptor::Abarrier(async_comm).req();
        }
...
VisMF::WriteAsyncMPIOneSidedFence (const FabArray<FArrayBox>& mf, const std::string& mf_name)
{
         ...
         -> WriteAsyncStatus
    {
        ...
        {
#ifdef BL_USE_MPI
            MPI_Win_fence(MPI_MODE_NOSTORE|MPI_MODE_NOPUT|MPI_MODE_NOSUCCEED, async_window);
            fence_count++;
#endif
        }
                           ...
                           int nlevels, bool set_ghost_cells, int hdr_proc)
{
    ...
    std::move(local_plotfile_data), std::move(hdrs), std::move(all_global_hdr_data));
