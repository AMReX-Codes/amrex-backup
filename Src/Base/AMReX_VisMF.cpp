...
VisMF::Initialize ()
{
    ...
    asyncTurn = std::malloc(sizeof(int));
...
VisMF::WriteAsyncMPIWait (const FabArray<FArrayBox>& mf, const std::string& mf_name)
{
         ...
         -> WriteAsyncStatus
    {
        ...
        if (ispot)
        {
            ...
            ParallelDescriptor::Arecv<int>(&myturn, sizeof(int), ispot-1, asyncTag, async_comm);
            icanwrite.wait();
        }
...
VisMF::WriteAsyncMPIOneSidedFence (const FabArray<FArrayBox>& mf, const std::string& mf_name)
{
         ...
         -> WriteAsyncStatus
    {
        ...
        // (after the appropriate number of calls) write, then get back to fence-ing.
