module amrex_eb_flux_reg_3d_module
  ...
  public :: amrex_eb_flux_reg_crseadd_va, amrex_eb_flux_reg_crseadd_v, amrex_eb_flux_reg_fineadd_va, &
       amrex_eb_flux_reg_fineadd_v, amrex_eb_flux_reg_fineadd_dm, amrex_eb_rereflux_from_crse, &
       amrex_eb_rereflux_to_fine
...
contains
  ...
  subroutine amrex_eb_flux_reg_crseadd_v (lo, hi, d, dlo, dhi, flag, fglo, fghi, &
       fx, fxlo, fxhi, fy, fylo, fyhi, fz, fzlo, fzhi, &
       vfrac, vlo, vhi, &
       dx, dt, nc) &
       ...
       do    j = lo(2), hi(2)
          do i = lo(1), hi(1)
             if (flag(i,j,k).eq.crse_fine_boundary_cell .and. vfrac(i,j,k).gt.1.d-14) then
                ...
                if (flag(i-1,j,k).eq.fine_cell) then
                   d(i,j,k,:) = d(i,j,k,:) - dtdx*fx(i,j,k,:)*volinv
                else if (flag(i+1,j,k).eq.fine_cell) then
                   d(i,j,k,:) = d(i,j,k,:) + dtdx*fx(i+1,j,k,:)*volinv
                end if

                if (flag(i,j-1,k).eq.fine_cell) then
                   d(i,j,k,:) = d(i,j,k,:) - dtdy*fy(i,j,k,:)*volinv
                else if (flag(i,j+1,k).eq.fine_cell) then
                   d(i,j,k,:) = d(i,j,k,:) + dtdy*fy(i,j+1,k,:)*volinv
                end if

                if (flag(i,j,k-1).eq.fine_cell) then
                   d(i,j,k,:) = d(i,j,k,:) - dtdz*fz(i,j,k,:)*volinv
                else if (flag(i,j,k+1).eq.fine_cell) then
                   d(i,j,k,:) = d(i,j,k,:) + dtdz*fz(i,j,k+1,:)*volinv
                end if
  ...
  end subroutine amrex_eb_flux_reg_crseadd_v
  ...
  subroutine amrex_eb_flux_reg_fineadd_v (lo, hi, d, dlo, dhi, f, flo, fhi, &
    ...
    ! dx is fine.  vfrac is fine as well.
    ! lo and hi are also relative to the fine box.
             ...
             do k = lo(3), hi(3)
                do j = lo(2), hi(2)
                   do koff = 0, ratio(3)-1
                      ...
                      do joff = 0, ratio(2)-1
                         jj = j*ratio(2)+joff
                         d(i,j,k,n) = d(i,j,k,n) - fac*f(ii,jj,kk,n)*cvol(i,j,k)
                      end do
                   end do
                end do
             end do
  ...
  end subroutine amrex_eb_flux_reg_fineadd_v
