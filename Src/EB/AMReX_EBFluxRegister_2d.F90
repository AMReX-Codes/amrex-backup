    ...
    do    j = lo(2), hi(2)
       do i = lo(1), hi(1)
          if (flag(i,j).eq.crse_fine_boundary_cell .and. vfrac(i,j).gt.1.d-14) then
             ...
             if (flag(i-1,j).eq.fine_cell) then
                d(i,j,:) = d(i,j,:) - dtdx*fx(i,j,:)*volinv
             else if (flag(i+1,j).eq.fine_cell) then
                d(i,j,:) = d(i,j,:) + dtdx*fx(i+1,j,:)*volinv
             end if
             
             if (flag(i,j-1).eq.fine_cell) then
                d(i,j,:) = d(i,j,:) - dtdy*fy(i,j,:)*volinv
             else if (flag(i,j+1).eq.fine_cell) then
                d(i,j,:) = d(i,j,:) + dtdy*fy(i,j+1,:)*volinv
             end if
       ...
       if (side .eq. 0) then ! lo-side
          ...
          do n = 1, nc
             do j = lo(2), hi(2)
                do joff = 0, ratio(2)-1
                   jj = j*ratio(2)+joff
                   d(i,j,n) = d(i,j,n) - fac*f(ii,jj,n)*cvol(i,j)
                end do
             end do
          end do
