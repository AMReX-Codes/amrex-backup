		 ...
		 SDCstruct &SDC, Real a, Real d, Real r)
{
  ...
  */
  ...
  // includes periodic domain boundaries (probably not necessary)
  ...
  SDC_fhigh_eval(flux,geom,bc,SDC,a,d,r,sdc_m);
  //SDC_fhigh_eval(flux,geom,bc,SDC,a,d,r,sdc_m);
  // Copy first function value to all nodes
    // Why? First iteration is constant in time: i.e. phi_0 = phi^0
  for (int sdc_n = 1; sdc_n < SDC.Nnodes; sdc_n++)
    {
      ...
      MultiFab::Copy(SDC.fhigh[sdc_n],SDC.f[1][0], 0, 0, 1, 0);
      if (SDC.Npieces==3)
  ...
  for (int k=1; k <= SDC.Nsweeps; ++k)
    {
      ...
      // If doing a calculation with residual shouldn't do this, as this will just lead to more computation than necessary.
      // Saved in res. Need to be careful about where else this is used. Else you can just be lazy and compute both 4th and second order stencils.
      ...
      for (int sdc_m = 0; sdc_m < SDC.Nnodes-1; sdc_m++)
	{
      ...
      SDC_fhigh_eval(flux,geom,bc,SDC,a,d,r,sdc_m+1);
...
void SDC_fhigh_eval(std::array<MultiFab, AMREX_SPACEDIM>& flux,
               const Geometry& geom,
               const Vector<BCRec>& bc,
               SDCstruct &SDC,
               Real a,Real d,Real r,
               int sdc_m)
{
    ...
    for ( MFIter mfi(SDC.sol[sdc_m]); mfi.isValid(); ++mfi )
    {
            ...
            SDC_fhigh_eval_F(BL_TO_FORTRAN_BOX(bx),
                        BL_TO_FORTRAN_BOX(domain_bx),
                        BL_TO_FORTRAN_ANYD(SDC.sol[sdc_m][mfi]),
                        BL_TO_FORTRAN_ANYD(flux[0][mfi]),
                        BL_TO_FORTRAN_ANYD(flux[1][mfi]),
                        ...
                        BL_TO_FORTRAN_ANYD(flux[2][mfi]),
#endif
                        BL_TO_FORTRAN_ANYD(SDC.fhigh[sdc_m][mfi]),
                        dx,&a,&d,&r);
