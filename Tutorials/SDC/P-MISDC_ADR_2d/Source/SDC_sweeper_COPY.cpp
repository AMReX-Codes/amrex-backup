  ...
  if (npiece == 1)  
    {
      ...
      mlabec.setLevelBC(0, &SDC.sol[sdc_m]); // What are these things doing?
      int resk=0;
      int maxresk=10;
      while ((resnorm > tol_res) & (resk <=maxresk))
	{
	  ...
	  mlabec.setLevelBC(0, &resid); // Opposite order from last time?
	  mlmg.setFixedIter(3);	  // Probably don't want this.
