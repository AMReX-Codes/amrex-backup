...
void main_main ()
{
	...
	if (plot_err == 1)  // Turn the solution into the error
	  {
		...
		err_phi(BL_TO_FORTRAN_BOX(bx),
			BL_TO_FORTRAN_ANYD(phi_new[mfi]),
			geom.CellSize(), geom.ProbLo(), geom.ProbHi(),&d,&time);
	      }
	  }
	int n = 0;
  ...
  */
    ...
    amrex::Print() << "Testing 2" << "\n";
    BccCoef.setVal(1.0);
    amrex::Print() << "Testing 3" << "\n";
  ...
  for (int n = 1; n <= Nsteps; ++n)
    {
      ...
      // Do an SDC step
      SDC_advance(phi_old, phi_new,flux, dt, geom, bc, mlmg,mlabec,SDCmats,d); 
